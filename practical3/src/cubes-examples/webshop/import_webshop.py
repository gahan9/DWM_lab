...
def import_sales():
    count = 0
    header = None
    with open('webshop-facts.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            count = count + 1
            fact = {"id": count}
            ...
            # Process row
            fact["date_id"] = insert_date(arow["date_created.year"], arow["date_created.month"],
                                          arow["date_created.day"])
    ...
    print("Imported {} facts ".format(count))
